...
int main() {
    ...
    //for (int seed: seeds) cout << node2res[seed] << " " ;
    //cout << endl;
    ...
    reverse(ts.begin(), ts.end());
    //for (int x: ts) cout << node2res[x]<< endl;

    vector<int> ans(N, -1);

    for (int node: ts) {
        int res = node2res[node];

        for (ii p: res2pair[res]) {
            int p_idx = pairidx[p];
            if (ans[p_idx] == -1) {
                ans[p_idx] = opsthatgivesthatres(res, p);
                break;
            }
        }
    }

    for (int a: ans) if (a == -1) {
        cout << "impossible" << endl;
        return 0;
    }
